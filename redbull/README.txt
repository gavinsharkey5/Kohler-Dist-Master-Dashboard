...
Categories (per Kohler, updated 2026-09-11 -- Sugar Free split out of
Core into its own "Free" tag):
  Core   Red Bull 1/24/8.4 oz Can (Regular only)
  Free   Red Bull Sugar Free, Red Bull Sugar Free Watermelon
...
Files:
  data.csv        Customer Name, Category (Core/Free/Core+), Sales Rep,
                  ...
                  no embedded JSON.
  goals.csv       Category, Goal (Core/Free/Core+/Overall). Also read
...
To refresh with a new export:
  ...
  3. If it errors on an unrecognized product, confirm with the user
     whether it's Core, Free or Core+, then add it to CORE_PRODUCTS /
     FREE_PRODUCTS / COREPLUS_PRODUCTS at the top of generate.py.
  4. Commit and push.
...
script (key / label / tag CSS class / blurb); everything else on the
page -- stat tiles, leaderboard lines, category cards, account tags,
headline and footer -- is generated from those two lists.
